...
import httpx
import asyncio
from consistent_hash import ConsistentHashRing  # Переконайся, що цей файл є поруч!
import os
...
# === ДОПОМІЖНА ФУНКЦІЯ (Винесена назовні) ===
async def trigger_rebalance_on_all(active_shards: List[str]):
    """
    Відправляє команду /rebalance на всі активні шарди.
    """
    logger.info(f"⚖️ Triggering rebalance for topology: {active_shards}")
    async with httpx.AsyncClient() as client:
        # Відправляємо запит кожному шарду (fire and forget)
        tasks = []
        for shard_name in active_shards:
            # Формуємо URL до шарда
        ...
        # Чекаємо відправки (але не обов'язково чекаємо завершення міграції на шардах)
        results = await asyncio.gather(*tasks, return_exceptions=True)
        for i, res in enumerate(results):
            if isinstance(res, Exception):
                logger.warning(f"⚠️ Failed to trigger rebalance on {active_shards[i]}: {res}")


# === АВТО-ВИЯВЛЕННЯ (Виправлено) ===
        ...
        try:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if group_name not in SHARD_GROUPS:
                            logger.info(f"🚀 Discovered NEW shard: {group_name}")

                            # 1. Реєструємо локально
                            ...
                            # 2. ТРИГЕРИМО РЕБАЛАНС (Викликаємо глобальну функцію)
                            # Список активних шардів = ті, що вже були + новий
            ...
            for group in existing_groups:
                if group.startswith("shard-node-") and group not in current_discovered:
                    logger.warning(f"🔻 Shard LOST: {group}. Removing from ring.")
